...
const ViewCompVisit = ({ route }) => {
  ...
  const handleItemPress = (item) => {
    navigation.navigate('CompVisitDetails', { faculty: item });
  };
  ...
  const handleDelete = async (item) => {
    try {
      const response = await fetch(`https://${variableValue}/compVisit/${item.id}`, {
        method: 'DELETE',
      });
