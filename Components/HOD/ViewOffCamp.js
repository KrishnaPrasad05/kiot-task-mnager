...
const ViewOffCamp = ({ route }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`https://${variableValue}/offCamp`); // Replace with your server URL
  ...
  const handleItemPress = (item) => {
    navigation.navigate('OffCampDetails', { faculty: item });
  };
  ...
  const onRefresh = async () => {
    ...
    try {
      const response = await fetch(`https://${variableValue}/offCamp`); // Fetch data
      const jsonData = await response.json();
  ...
  const handleDelete = async (item) => {
    try {
      const response = await fetch(`https://${variableValue}/offCamp/${item.id}`, {
        method: 'DELETE',
      });
