...
const AssignedOnCamp = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`https://${variableValue}/onCamp?assignTo=${name}`);
  ...
  const handleItemPress = (item) => {
    navigation.navigate('AssignedOnCampusDetails', { faculty: item });
  };
        ...
        <View style={styles.itemContent}>
          <View style={styles.textContainer}>
            ...
            <View style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexDirection:'row',width:'100%',flexWrap:'wrap'}}>
              <Text style={{marginRight:100,color:'grey'}}>Resolve By : {item.date} | {item.time}</Text>
