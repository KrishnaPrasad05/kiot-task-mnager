...
const ViewOnCamp = ({ route }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`https://${variableValue}/onCamp`); // Replace with your server URL
  ...
  const handleItemPress = (item) => {
    navigation.navigate('OnCampDetails', { faculty: item });
  };
  ...
  const onRefresh = async () => {
    ...
    try {
      const response = await fetch(`https://${variableValue}/onCamp`); // Fetch data
      const jsonData = await response.json();
  ...
  const handleDelete = async (item) => {
    try {
      const response = await fetch(`https://${variableValue}/onCamp/${item.id}`, {
        method: 'DELETE',
      });
